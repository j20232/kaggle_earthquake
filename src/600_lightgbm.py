...
def train_with_lightgbm():
    ...
    with config_file.open() as f:
        params = json.load(f)
    validity = None
    model_path = cc.MODEL_PATH / sys.argv[1]
    Path.mkdir(model_path, exist_ok=True, parents=True)
    
    importance_path = cc.IMPORTANCE_PATH / cc.PREF
    ...
    for fold, fold_dir in enumerate(fold_dir_list):
        ...
        print("\t >> {} folds finish: [elapsed_time] >> {:0>2}:{:0>2}:{:0>2}".format(fold, hour, minutes, sec))

        model.save_model(str(model_path / "fold{}.model".format(fold)))

        validity = cc.output_cv(validity)
        cc.save_feature_importance(feature_importance, importance_path)

        del train_dataset, valid_dataset
        gc.collect()
    oof_path = cc.OOF_PATH / cc.PREF
